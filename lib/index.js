...
function getManifestFromSpdxFile(document, fileName) {
    ...
    document.packages?.forEach(pkg => {
        ...
        try {
            ...
            let hasRootParent = document.relationships?.some(rel => rel.relatedSpdxElement == pkg.SPDXID && rel.relationshipType == "DEPENDS_ON" && rel.spdxElementId == "SPDXRef-RootPackage");
            let hasNonRootParent = document.relationships?.some(rel => rel.relatedSpdxElement == pkg.SPDXID && rel.relationshipType == "DEPENDS_ON" && rel.spdxElementId != "SPDXRef-RootPackage");
            if (hasNonRootParent && !hasRootParent) {
