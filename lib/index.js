...
function getSubmissionContext(defaultContext, workingDirectory = process.cwd()) {
    ...
    return {
        ...
        eventName: '',
        repo: {
            owner: core.getInput('owner') || defaultContext.repo.owner,
            repo
        },
        sha: core.getInput('repoSha') || getGitValue(['rev-parse', 'HEAD'], 'repoSha', workingDirectory),
        ref: core.getInput('repoRef') || getGitValue(['symbolic-ref', 'HEAD'], 'repoRef', workingDirectory)
    };
}
